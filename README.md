...
The backend handles the business rules, validation, persistence, portfolio valuation, and reporting. The frontend is a React + TypeScript single-page application that consumes the backend API and presents the system through routed pages.
...
Shared TypeScript DTO types are kept in the service layer so API response shapes are typed without adding global state or extra frontend architecture.
...
TypeScript is used mainly for API DTOs, service return types, component props, form events, and nullable state.
...
```text
void-ledger-client/vite.config.ts
```
...
- The frontend uses a custom domain connected to Azure Static Web Apps.
